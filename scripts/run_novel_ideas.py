...
def main() -> None:
    ...
    db.ingest_dataframe(
        run_id=run_id,
        script="run_novel_ideas.py",
        artifact_type="novel_strategy_summary",
    ...
    if robustness_rows:
        db.ingest_dataframe(
            run_id=run_id,
            script="run_novel_ideas.py",
            artifact_type="novel_strategy_robustness",
            source_path=str(robustness_csv_path),
            df=pl.DataFrame(robustness_rows),
        )
